...
if config.HAVE_FENICS:
    ...
    class FenicsNonaffineOperator(OperatorBase):
        ...
        def restricted(self, dofs):
            ...
            #  affected_cells = [df.Cell(mesh, ci) for ci in affected_cell_indices]# needed for
            #  UDEIM
            ...
            # define reduced quadrature spaces dependent on dimension
            mat_r = self.material.copy()
            mat_r.space = V_r_range
            scalar_space_r_range = df.FunctionSpace(submesh,
                                                    self.material.scalar_space.ufl_element())
            vector_space_r_range = False
            tensor_space_r_range = False
            mat_r.scalar_space = scalar_space_r_range
            # TODO: map history data to submesh ...
            if self.material.dim == 2:
                vector_space_r_range = df.FunctionSpace(submesh,
                                                        self.material.vector_space.ufl_element())
            elif self.material.dim == 3:
                tensor_space_r_range = df.FunctionSpace(submesh,
                                                        self.material.tensor_space.ufl_element())
            else:
                raise AttributeError
            mat_r.vector_space = vector_space_r_range
            mat_r.tensor_space = tensor_space_r_range
            mat_r.initialize_history()
            history_r = mat_r._history_data
            ...
            return (RestrictedFenicsNonaffineOperator(op_r, restricted_range_dofs),
                    source_dofs[np.argsort(restricted_source_dofs)])

    class RestrictedFenicsNonaffineOperator(OperatorBase):
        ...
        def apply(self, U, mu=None):
            assert U in self.source
            self.op._set_mu(mu)
            # self.op.material.update_history(U)
