...
def project_system(op, range_bases, source_bases, products=None):
    ...
    if system_in_range:
        assert all(rb is None or rb in ss for rb,
                   ss in zip(range_bases, op.range.subspaces))
        products = products or [None] * len(range_bases)
        assert all(p is None or p.source == p.range
                   == ss for p, ss in zip(products, range_bases))
    else:
    ...
    if system_in_source:
        assert all(sb is None or sb in ss for sb, ss in zip(
            source_bases, op.source.subspaces))
    else:
...
class ProjectSystemRules(RuleTable):
        ...
        if not op.adjoint:
            ...
            def inner(U, V, product=None):
                if V:
                    ...
                    # return block if range basis is None
                    return U.to_numpy()

            projected_array = BlockVectorSpace.make_array(
                [NumpyVectorSpace.from_numpy(inner(block, range_basis, product), block.space.id)
                 for block, range_basis, product in zip(op.array._blocks, range_bases, products)]
            )
        else:
            assert not system_in_range
            ...
            projected_array = BlockVectorSpace.make_array(
                [NumpyVectorSpace.from_numpy(block.dot(source_basis), block.space.id)
                 for block, source_basis in zip(op.array._blocks, source_bases)]
            )
        return VectorArrayOperator(projected_array, adjoint=op.adjoint, space_id=op.space_id,
                                   name=op.name)
    ...
    def action_Concatenation(self, op):
        ...
        if op.parametric or not op.linear:
            raise RuleNotMatchingError(
                'Only implemented for non-parametric linear Concatenations')
        ...
        if system_in_range:
            second_blocks = np.full(
                (len(second.range.subspaces), blocks.shape[1]), None)
        else:
        ...
        for j in range(second_blocks.shape[1]):
            if not second_system_in_source:
                ...
                if not isinstance(sb, ZeroOperator):
                    ...
                    projected_second = project_system(
                        second, range_bases, sb, products=products)
